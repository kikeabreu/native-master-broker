...
export default function CRM({ user, role = "team" }) {
  ...
  const saveTarea = async (t) => {
    ...
    try {
      ...
      console.log("TAREAS CARGADAS:", tList);
    } catch (e) {
      console.error(e);
  ...
  const delTarea = async (id) => {
    ...
    // 2) BD
    try {
      const { dbDeleteTask, dbGetTasks } = await import("./db");
  ...
  const addAct = async (a) => {
    // 1) UI inmediato
    const n = [...acts, { id: uid(), fecha: today(), ...a }];
    setActs(n);
    ...
    try {
      ...
      // payload a BD (sin id: BD genera UUID)
      ...
      // refresco para agarrar IDs reales
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
        ...
        <div style={S.card}>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead><tr><th style={S.th}>Tipo</th><th style={S.th}>Cant.</th><th style={S.th}>Fecha</th><th style={S.th}>Notas</th></tr></thead>
            <tbody>
              {acts.slice().reverse().slice(0, 20).map(a => {
                const at = ACT_TYPES.find(x => x.id === a.tipo) || { icon: "⚡", label: a.tipo }; return (
                  <tr key={a.id}><td style={S.td}><span style={S.bdg("#6366f1")}>{at.icon} {at.label}</span></td><td style={{ ...S.td, fontWeight: 700, color: "#fff" }}>{a.cantidad || 1}</td><td style={{ ...S.td, fontSize: 12, color: "#6b7280" }}>{fd(a.fecha)}</td><td style={{ ...S.td, fontSize: 12, color: "#4b5563" }}>{a.notas || "—"}</td></tr>
                );
              })}
              {acts.length === 0 && <tr><td colSpan={4} style={{ ...S.td, textAlign: "center", color: "#374151", padding: 28 }}>Sin actividades registradas</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
    ...
    <div style={S.app}>
      ...
      <div style={S.side}>
        <div style={S.logo}>
          ...
          <div style={{ fontSize: 19, fontWeight: 900, color: "#fff", letterSpacing: -.5 }}>Top Seller</div>
          <div style={{ fontSize: 9, color: "#fff", marginTop: 1 }}>CRM · Área Comercial v4</div>
          <button
      ...
      <div style={S.main}>
        ...
        <div style={S.page}>
          ...
          {view === "reporte" && (role === "admin" ? <Reporte /> : <div style={{ color: "#fff" }}>Sin acceso</div>)}
