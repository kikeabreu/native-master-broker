...
const kanbanToStages = (tab, target) => {
  ...
  const ns={}; stages.slice(0,fill).forEach(st=>ns[st]=true);
...
export default function CRM({ user, role="team" }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setActs(aList);
        setProfiles(pRows||[]);
        if (c) setCfg({ ...DEFAULT_CONFIG, agencia:c.agencia, socios:c.socios, moneda:c.moneda, metaClientes:c.meta_clientes, metaIngresos:Number(c.meta_ingresos), inversion:Number(c.inversion), periodoInicio:(c.periodo_inicio||"").slice(0,10), periodoFin:(c.periodo_fin||"").slice(0,10) });
  ...
  const saveP = async (tab, p) => {
    // OWNER FIX: preserve original owner, only set current user for new prospects
    const existingOwner = allP.find(x=>x.id===p.id)?.owner_id;
    const ownerId = existingOwner || user.id;
    ...
    try {
      ...
      const conocePorVal = Array.isArray(u.conocePor) ? u.conocePor : (u.conocePor ? [u.conocePor] : []);
  ...
  const toggleStage = async (tab,pid,stage) => {
    ...
    if (p) { try { const { dbUpsertProspect }=await import("./db"); const existingOwner=allP.find(x=>x.id===pid)?.owner_id||user.id; await dbUpsertProspect({ id:pid, tab, owner_id:existingOwner, data:Object.fromEntries((FIELDS[tab]||[]).map(k=>[k,p[k]??""])), stages:p.stages||{}, perdido:!!p.perdido, venta:Array.isArray(p.venta)?p.venta:(p.venta?.monto?[p.venta]:[]), notas_historial:p.notasHistorial||[] }); } catch(e){ console.error(e); } }
  };

  const saveTarea = async (t) => {
    const n = tareas.find(x=>x.id===t.id) ? tareas.map(x=>x.id===t.id?t:x) : [...tareas,t];
    setTareas(n);
    try {
  ...
  const addAct = async (a) => {
    const optimistic=[...acts,{id:uid(),fecha:a.fecha||today(),...a,ownerId:user.id,ownerName:user?.user_metadata?.full_name||"—",ownerRole:role||"team"}];
    setActs(optimistic);
    try {
      const { dbInsertActivity, dbGetActivities }=await import("./db");
      await dbInsertActivity({ owner_id:user.id, tipo:a.tipo, cantidad:Number(a.cantidad||1), fecha:a.fecha||today(), notas:a.notas||"" });
      const rows=await dbGetActivities();
      const list=(rows??[]).map(row=>({ id:row.id, tipo:row.tipo, cantidad:row.cantidad, fecha:(row.fecha||"").slice(0,10), notas:row.notas||"", ownerId:row.owner_id||"", ownerName:row.owner_name||row.ownerName||"—", ownerRole:row.owner_role||row.ownerRole||null }));
  ...
  const saveCfg = async (c) => {
    ...
    try {
      ...
      await dbUpdateConfig({ agencia:c.agencia||"Native Master Broker", socios:c.socios||"", moneda:c.moneda||"MXN", meta_clientes:Number(c.metaClientes||10), meta_ingresos:Number(c.metaIngresos||50000), inversion:Number(c.inversion||0), periodo_inicio:c.periodoInicio, periodo_fin:c.periodoFin });
      const row=await dbGetConfig();
      setCfg({ ...DEFAULT_CONFIG, agencia:row.agencia, socios:row.socios, moneda:row.moneda, metaClientes:row.meta_clientes, metaIngresos:Number(row.meta_ingresos), inversion:Number(row.inversion), periodoInicio:(row.periodo_inicio||"").slice(0,10), periodoFin:(row.periodo_fin||"").slice(0,10) });
    ...
    useEffect(()=>{
      (async()=>{
        try {
          ...
          // Enriquecer con facturación desde allP
          const facByOwner={};
...
IMPORTANTE: Esta acción es permanente. Asegúrate de hacerlo intencionalmente.
  ...
  const ProspectModal = () => {
    ...
    const [form,   setForm]   = useState(()=> modal.person ? {...modal.person,tab} : {id:uid(),tab,stages:{},notasHistorial:[],creadoEn:today(),updatedAt:today()});
    const [tab_,   setTab_]   = useState("datos");
    const [nNota,  setNNota]  = useState("");
    const [ventas, setVentas] = useState(initVentas);
    const [newTask,setNewTask]= useState(null);

    // conocePor como array
    const conocePorArr = Array.isArray(form.conocePor)?form.conocePor:(form.conocePor?[form.conocePor]:[]);
    const toggleConoce = (opt) => {
      const arr=conocePorArr.includes(opt) ? conocePorArr.filter(x=>x!==opt) : [...conocePorArr,opt];
      setForm(f=>({...f,conocePor:arr}));
    };
    ...
    const addVenta = () => setVentas(v=>[...v,{id:uid(),desarrollo:"",monto:"",tipo:"contado",fechaInicio:"",engancheDiferido:false}]);
    ...
    const handleSave = () => saveP(tab, {...form, venta:ventas});

    const PROSPECT_TABS=[["datos","👤 Datos"],["etapas","📊 Etapas"],["notas","📝 Historial"],["tareas","✅ Tareas"],["venta","💰 Ventas"]];
        ...
        <div style={{...S.mbox,maxWidth:640}}>
          ...
          <div style={{display:"flex",gap:4,padding:"10px 20px",borderBottom:"1px solid #1a1a2e",flexShrink:0,overflowX:"auto"}}>
            {PROSPECT_TABS.map(([id,label])=>(
              <button key={id} style={{...S.btn(tab_===id?"primary":"secondary",tab_===id?color:undefined),padding:"5px 12px",whiteSpace:"nowrap"}} onClick={()=>setTab_(id)}>{label}{id==="tareas"&&linkedTareas.length>0&&<span style={{marginLeft:5,background:"rgba(255,255,255,.2)",borderRadius:10,padding:"0 5px",fontSize:9}}>{linkedTareas.length}</span>}</button>
            ))}
          </div>
          ...
          <div style={{padding:20,overflow:"auto",flex:1}}>
            ...
            {tab_==="datos"&&(
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                ...
                {(FIELDS[tab]||[]).includes("conocePor")&&(
                  <div>
                    <label style={S.lbl}>¿De dónde lo conozco? (selección múltiple)</label>
                    <div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:4}}>
                      {CONOCE_POR_OPTIONS.map(opt=>{
                        const on=conocePorArr.includes(opt);
                        return (<button key={opt} style={{padding:"6px 14px",borderRadius:20,border:`1px solid ${on?color:color+"44"}`,background:on?color+"22":"transparent",color:on?"#fff":"#6b7280",fontSize:12,fontWeight:on?700:400,cursor:"pointer",transition:"all .15s"}} onClick={()=>toggleConoce(opt)}>{on?"✓ ":""}{opt}</button>);
                      })}
                    </div>
                  </div>
                )}
            ...
            {tab_==="venta"&&(
              <div style={{display:"flex",flexDirection:"column",gap:16}}>
                ...
                {ventas.map((v,i)=>(
                  <div key={v.id} style={{background:"#07070f",border:"1px solid #10b98133",borderRadius:10,padding:16}}>
                    ...
                    <div><label style={S.lbl}>Desarrollo / Proyecto</label><input style={S.inp} value={v.desarrollo||""} onChange={e=>updVenta(v.id,"desarrollo",e.target.value)} placeholder="Cayo Coco, Palmarena, Gran Puerto..."/></div>
                    <div style={{...S.g2,marginTop:10}}>
                      <div><label style={S.lbl}>Monto de venta ($)</label><input type="number" style={S.inp} value={v.monto||""} onChange={e=>updVenta(v.id,"monto",e.target.value)}/></div>
                    ...
                    <div style={{...S.g2,marginTop:10}}>
                      <div><label style={S.lbl}>Fecha de cierre</label><input type="date" style={S.inp} value={v.fechaInicio||""} onChange={e=>updVenta(v.id,"fechaInicio",e.target.value)}/></div>
                      <div style={{display:"flex",alignItems:"center",gap:10,paddingTop:20}}>
                        <Chk on={!!v.engancheDiferido} color="#f59e0b" onChange={()=>updVenta(v.id,"engancheDiferido",!v.engancheDiferido)}/>
                        <span style={{fontSize:13,color:"#9ca3af"}}>Enganche diferido</span>
                      </div>
                    </div>
                    {v.monto&&<div style={{marginTop:12,fontSize:12,color:"#10b981",fontWeight:700}}>💰 Monto: {fm(Number(v.monto),"MXN")}</div>}
                  </div>
                ))}
    ...
    <div style={S.app}>
      ...
      <div style={S.main}>
        ...
        <div style={S.page}>
          ...
          {view==="notificaciones"&&<NotificationsPanel userId={user?.id} userRole={role} profiles={profiles}/>}
        </div>
      </div>
